...
lnaddr_t seg_translate(swaddr_t eip,uint32_t len, uint8_t sreg);
uint32_t swaddr_read(swaddr_t, size_t, uint16_t);
uint32_t lnaddr_read(lnaddr_t, size_t);
uint32_t hwaddr_read(hwaddr_t, size_t);
void swaddr_write(swaddr_t, size_t, uint32_t, uint16_t);

...
/*
	take 0x80 as example ,
	static helper_fun opcode_table_group_b [8] = {inv,inv,inv,inv,inv,inv,inv,inv}
	static int group1_b(swaddr_t eip) {
		ModR_M m;
		m.val = instr_fetch(eip + 1,1);
		return opcode_table_group1_b[m.opcode](eip);
	}
	here we know m is 8bits type ,and val is also a 8bits type,so this union is filled by ans returned 
	from instr_fetch. m.opcode is m[3-5] thus we get a func from opcode_table_group1_b;
	we could conclude that group means opcode is not the only factor to decide which function to excute,
	as we should combine it with the m.opcode which is the opcode in ModR/M 
*/
/* 0x80 */
make_group(group1_b,
...
make_group(group2_i_b,
	inv, inv, inv, inv, 
	shl_rm_imm_b, shr_rm_imm_v, inv, sar_rm_imm_b)
...
make_group(group2_1_b,
	inv, inv, inv, inv, 
	shl_rm_1_b, shr_rm_1_v, inv, sar_rm_1_b)
...
/* 0x74 */	jz_i_b, jne_i_b, jbe_i_b, ja_i_b,
...
/* 0x08 */	inv, inv, inv, ud2, 
/* 0x0c */	inv, inv, inv, inv, 
/* 0x10 */	inv, inv, inv, inv, 
/* 0x14 */	inv, inv, inv, inv, 
/* 0x18 */	inv, inv, inv, inv, 
/* 0x1c */	inv, inv, inv, inv, 
/* 0x20 */	mov_c2r, inv, mov_r2c, inv, 
...
/* 0x4c */	inv, cmovge_rm2r_v, cmovle_rm2r_v, cmovg_rm2r_v, 
...
/* 0x84 */	jz_i_v, jne_i_v, jbe_i_v, ja_i_v,
...
/* 0xb4 */	inv, inv, movzx_rm2r_v, movzx_rm2r, 
/* 0xb8 */	inv, inv, bt_si2rm_v, inv,
/* 0xbc */	inv, inv, movsx_rm2r_v, movsx_rm2r,
...
/*
 get a byte from memory,and we could get a func to explain the opcode when the byte  exactly represent
 the opcode.For example ,when opcode is 0x88,we get the "mov_r2rm_b" func,so we call the function mov_r2rm_b
 (eip)
 */
make_helper(exec) {
	//printf("-3\n");

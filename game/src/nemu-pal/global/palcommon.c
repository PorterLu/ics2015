...
--*/
{
   ...
   fseek(fp, 0, SEEK_END);
   iNumChunk = ftell(fp);//fread(&iNumChunk, sizeof(INT), 1, fp);
   //read(&iNumChunk, sizeof(INT))
...
--*/
{
   ...
   printf("step 1\n");
   Log("lpBuffer:%x fp:%x uiBufferSize:%d",lpBuffer,fp,uiBufferSize);
   ...
   printf("step 2\n");
   uiChunkCount = PAL_MKFGetChunkCount(fp);
   Log("uiChunkNum:%d  uiChunkCount:%d\n",uiChunkNum,uiChunkCount);
   if (uiChunkNum >= uiChunkCount)
   {
      return -1;
   }
   ...
   uiOffset = SWAP32(uiOffset);
   uiNextOffset = SWAP32(uiNextOffset);
   Log("uiChunkNum:%d uiOffset:%d uiNextOffset:%d",uiChunkNum,uiOffset, uiNextOffset);
   printf("step 4\n");
   //
